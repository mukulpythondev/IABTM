...
export const addToCart = async (req, res) => {
    ...
    if (!userId) {
        return res.status(200).json(new ApiResponse(400, "user id not found"));
    }
    if (!productId || !quantity) {
        return res.status(200).json(new ApiResponse(400, "Product ID and quantity are required"));
    }
...
export const getCart = async (req, res) => {
    ...
    try {
        ...
        if (!cart) {
            return res.status(200).json(new ApiResponse(400,"Cart not found"));
        }
